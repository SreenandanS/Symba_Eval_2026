"""Models for physics-informed QED amplitude -> squared-amplitude decoding."""
...
from .encoder import CustomQEDAmp2SqEncoder, FlatSequenceEncoder
from .factorization import TARGET_VARIANT_FACTORIZED, normalize_target_variant
...
@dataclass
class QEDAmp2SqModelOutput:
    sequence_logits: Tensor


@dataclass
class QEDAmp2SqGeneration:
    sequence_ids: Tensor


class QEDAmp2SqModel(nn.Module):
    def __init__(
        self,
        vocab_size: int,
        node_in_dim: int,
    ...
    def _build_encoder(
        *,
        ...
        raise ValueError(
            f"Unknown encoder_variant '{encoder_variant}'. Expected 'custom' or 'seq2seq'."
        )
    ...
    def _full_sequence_memory(
        ...
        raw_token = self.full_raw_context(
            torch.cat([pooled["pooled"], pooled["raw_pool"], pooled["global_pool"]], dim=-1)
        ).unsqueeze(1)
        feature_token = self.full_feature_context(
            torch.cat(
                [pooled["pooled"], pooled["feature_pool"], pooled["global_pool"]],
                dim=-1,
            )
        ).unsqueeze(1)
        family_token = self.full_family_context(
            torch.cat(
                [pooled["pooled"], pooled["family_pool"], pooled["feature_pool"]],
                dim=-1,
            )
        ).unsqueeze(1)
    ...
    def forward(
        ...
        sequence_logits = _ensure_finite("QEDAmp2SqModel.decoder", sequence_logits)
        return QEDAmp2SqModelOutput(sequence_logits=sequence_logits)
    ...
    def generate(
        ...
        return QEDAmp2SqGeneration(sequence_ids=sequence_ids)
