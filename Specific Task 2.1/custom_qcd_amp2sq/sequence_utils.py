"""Typed source-token serialization for canonical amplitudes."""
...
def _normalize_raw_source_tokens(tokens: Sequence[str]) -> list[str]:
    ...
    for token in tokens:
        if _RAW_MOMENTUM_TOKEN_RE.fullmatch(token):
            mapped = momentum_map.setdefault(
                token,
                f"MOMENTUM_{len(momentum_map)}",
            )
            normalized.append(mapped)
            continue

        if (
            token in {"g", "i", "reg_prop"}
        ...
        if match is not None:
            ...
            mapped = index_map.setdefault(
                token,
                f"{prefix}_INDEX_{len(index_map)}",
            )
            normalized.append(mapped)
            continue
