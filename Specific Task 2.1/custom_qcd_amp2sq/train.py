"""Training utilities for the standalone amplitude -> squared-amplitude package."""
...
from .config import Amp2SqConfig
from .dataset import (
    Amp2SqBatch,
    Amp2SqDataset,
...
from .model import Amp2SqModel
from .tokenizer import AmplitudeTokenizer


@torch.no_grad()
def numerator_token_accuracy(logits: Tensor, targets: Tensor, pad_id: int) -> float:
...
class Amp2SqTrainer:
    def __init__(self, config: Amp2SqConfig):
        self.config = config
        ...
        self.cache_dir = self.output_dir / "cache" / "amp2sq_compiled_v1"
        self.cache_dir.mkdir(parents=True, exist_ok=True)

        self.corpus = load_amp2sq_corpus(
    ...
    def _loss_and_metrics(
        ...
        metrics = {
            "token_acc": numerator_token_accuracy(
    ...
    def train(self) -> Dict[str, object]:
        cfg = self.config
        print(f"\n{'=' * 60}")
        print(f"  Amp2Sq Training: {cfg.experiment_name}")
        print(f"{'=' * 60}")
        print(f"  Encoder: {cfg.model.encoder_variant}")
        print(
            "  Source view: "
            + (
                "physics-tagged amplitude sequence"
