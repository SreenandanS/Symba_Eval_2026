"""Training utilities for the QCD custom fd-to-squared-amplitude suite."""
...
class CustomQCDFd2SqTrainer:
    def __init__(self, config: CustomQCDFd2SqConfig):
        self.config = config
        ...
        if config.model.encoder_variant == "seq2seq":
            ...
            edge_in_dim = (
                sample_batch.edge_attr.size(1)
                if sample_batch.edge_attr.numel() > 0
                else config.model.edge_in_dim
            )

        self.model = CustomQCDFd2SqModel(
    ...
    def train(self) -> Dict[str, object]:
        cfg = self.config
        print(f"\n{'=' * 60}")
        print(f"  Custom Training: {cfg.experiment_name}")
        print(f"{'=' * 60}")
